...
class Barista {
    ...
    func makeCoffee(menu: CoffeeMenu) -> Coffee {
        ...
        switch menu {
        case .americano:
            return Coffee.init(name: "아메리카노", price: 2000)
        case .latte:
            return Coffee.init(name: "라떼", price: 2500)
        default:
            return Coffee.init(name: "카푸치노", price: 3000)
        }
    }
...
class Customer {
    ...
    func order(menu: CoffeeMenu, to barista: Barista) {
        ...
        print("\(self.name)이(가) \(barista.name)에게 \(barista.makeCoffee(menu: menu).price)원짜리 \(barista.makeCoffee(menu: menu).name)을(를) 주문하였습니다.")
...
func arr(arr:Array<Int>) {
    ...
    for x in dic.keys.sorted() {
        print("Group :",x, "- Value: ",dic[x]!)
    }
}
...
//2)
var arr2 = ["1", "2", "3", "4", "5", "A", "B", "C", "D"]
print(arr2.compactMap{Int($0) != nil ? $0 : nil }.map{Int($0)! * Int($0)!}.reduce(0,+))
